...
pub const WaveformData = struct {
    ...
    fn updateMaximiums(waveform: *Self) !void {
        ...
        const _it = struct {
            fn iterator(
                ...
                const max: f32 = @as(*f32, @ptrCast(@alignCast(@constCast(user_data)))).*;
        ...
        waveform.rolling_maximums.iterateBackwards(_it.iterator, max_ptr);
    }
    ...
    pub fn create(buffer_length: usize, num_maximums: usize) !*WaveformData {
        const waveform_data = try std.heap.page_allocator.create(WaveformData);

        const buffer = try std.heap.page_allocator.alloc(f32, buffer_length);
        @memset(buffer, 0);
    ...
    pub fn updateFrequencyWaveform(
        ...
        try realFFT(raw_waveform_data.buffer.ptr, frequency_waveform_data.buffer.ptr, raw_waveform_data.buffer.len, lsa, lsb);
    }
    ...
    fn computeRMS(
        ...
        var sq_sum: f32 = 0;
        for (waveform_data.buffer) |val| {
            sq_sum += val * val;
        }

        const ms: f32 = sq_sum / @as(f32, @floatFromInt(waveform_data.buffer.len));

        return std.math.sqrt(ms);
    }

    pub fn computeDecibel(
        waveform_data: *Self,
    ) callconv(.c) f32 {
        const rms = waveform_data.computeRMS();

        return 20 * std.math.log10(rms);
    }
};

fn computeLogScaleIndex(a: f32, k: f32, index: usize) usize {
    const index_f32: f32 = @floatFromInt(index);

    const new_index: f32 = std.math.floor(a * std.math.exp(k * index_f32) - a);
...
pub fn realFFT(
    ...
    for (0..N / 2) |i| {
        output[i] = std.math.pow(f32, cx_output[computeLogScaleIndex(lsa, lsb, i)].magnitude(), 0.4);
    }
}
